...
let semaphore = function semaphore() {
    ...
    this.callDone = () => {
        ...
        //   console.log('call done called with:', this._doneCounter )
        // console.trace("Here I am!")
        if (!this._options.doneAmount) {
            //        console.log('there is no options yet');
            return;
        }
        //     console.log(`oprtion is already set`);
        this.__callPromise();
        //  console.log(`callDone:${doneCounter}`);
    }

    this.__callPromise = () => {
        //      console.log(`call promise called current counter is ${this._doneCounter} from ${this._options.doneAmount} `);
        if (this._doneCounter >= this._options.doneAmount) {
            // this._doneCounter = 0;
            //        console.log(`calling promise resolve. promise is: ${this._promise}`);
    ...
    this.done = async (options = { doneAmount: 1 }) => {
        ...
        //   console.log(`done :options is set with `, this._options.doneAmount);
        if (this._doneCounter >= this._options.doneAmount) {
